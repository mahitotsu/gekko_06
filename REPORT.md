...
- 今回の検証範囲では「明示的にkillするまで残り続けた」ケースは確認されなかった。ただし、これは
  SDKが正常系・中断系ともにクリーンな終了シーケンス(stdinクローズ等によるMCPサーバー側の自発終了、
  もしくはSDK側からのプロセス終了)を辿れた場合の結果であり、ネットワーク越しのSSE/HTTP MCPサーバーや、
  プロセスがSIGTERM/stdin-closeを無視するような実装の場合の挙動は本検証の対象外。
